If you want to install the setupsup module or if you want to build it from the scratch,
you should read these notes.


Installation:

Unzip the setupsup.1.0.1.0.zip file to a folder. Make this folder your current directory.
In contrast to the installation procedure for the module version 1.08, you don't need to
distinguish between perl 5.6 and perl 5.005. The Win32-setupsup.ppd file does this job for
you (thanks for the hint to Jenda Krynicky). Type in 

	ppm install win32-setupsup --location=.

if you use perl 5.6 or perl 5.005 (type in perl -v to verify your perl version). To verify 
the installation, type in ppm query. The setupsup module should appear in the list.
...
The html help file will be installed only if you use the ppm. You can find the setupsup.html
file in the <perldir>\html\lib\site\setupsup directory.



Build the module:

You need an installed MSVC++ compiler. I work with MSVC++ version 6 but version 5 should
work too. Next, you need the platform sdk. It contains some header files and libraries
necessary to build the module. Furthermore, you need the perl source files (in particular,
only the header files are necessary). These include header files should be accessible in
the <perldir>\lib\core directory. 

Set up the compiler environment with vcvars32.bat. Unzip the setupsup.1.0.1.0.zip file to an empty 
...
Change the following settings to your directory names:
	...
	perldir6xx=c:\programme\perl.615	- directory where perl 5.6 resides
	perldir5xx=c:\programme\perl.522	- directory where perl 5.005_3 resides
	perldir3xx=c:\programme\perl.316	- directory where perl 5.003 resides
